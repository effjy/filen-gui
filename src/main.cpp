// Filen GUI - a GTK4 desktop client for Filen.io
...
#define APP_ID "io.github.jflc.FilenGUI"
#define APP_VERSION "1.0"

namespace {

struct PathEntry { std::string uuid; std::string name; };
...
struct App {
    ...
    std::vector<filen::Item> items;       // current folder contents
};
...
void loadFolder(const std::string& uuid);
void rebuildPathLabel();
void refreshTrashState();

// ---- file download --------------------------------------------------------
struct SaveCtx { filen::Item item; };
...
void downloadItem(const filen::Item& item) {
...
void onRowActivated(GtkListBox*, GtkListBoxRow* row, gpointer) {
    ...
    const filen::Item& item = g_app->items[idx];
    // Folders open on activation; files just stay selected, waiting for the
    // Download or Delete toolbar action.
    if (item.isFolder) {
        g_app->path.push_back({item.uuid, item.name});
        loadFolder(item.uuid);
    }
}
...
GtkWidget* makeRow(const filen::Item& item) {
...
void loadFolder(const std::string& uuid) {
    ...
    std::thread([uuid]() {
        std::vector<filen::Item> items;
        std::string err;
        try {
            items = g_app->client.listDir(uuid);
        } catch (const std::exception& e) {
            err = e.what();
        }
        postToMain([items, err]() {
            ...
            std::sort(g_app->items.begin(), g_app->items.end(),
                      [](const filen::Item& a, const filen::Item& b) {
...
void onBackClicked(GtkButton*, gpointer) {
    ...
    g_app->path.pop_back();
    loadFolder(g_app->path.back().uuid);
}

void onRefreshClicked(GtkButton*, gpointer) {
    if (!g_app->path.empty()) loadFolder(g_app->path.back().uuid);
}
...
void onOpenReady(GObject* source, GAsyncResult* res, gpointer) {
    ...
    std::string parent = g_app->path.back().uuid;
    std::string base = src;
    ...
    std::thread([parent, src, base]() {
        ...
        postToMain([base, err, parent]() {
            ...
            if (err.empty()) {
                ...
                if (!g_app->path.empty() && g_app->path.back().uuid == parent)
                    loadFolder(parent);
            } else {
...
void doTrash(const filen::Item& item) {
    std::string parent = g_app->path.empty() ? "" : g_app->path.back().uuid;
...
void onTrashConfirm(GObject* source, GAsyncResult* res, gpointer user_data) {
    ...
    // Button 0 = Cancel, 1 = Move to Trash.
...
void onDeleteClicked(GtkButton*, gpointer) {
    ...
    gtk_alert_dialog_set_detail(dlg,
        item.isFolder ? "The folder and its contents will be moved to the Filen trash."
                      : "The file will be moved to the Filen trash.");
...
// Queries the trash in the background and enables/disables the empty-trash
// button accordingly.
void refreshTrashState() {
    std::thread([]() {
        int count = -1;
        try {
            count = g_app->client.trashCount();
        } catch (const std::exception&) { /* leave disabled on error */ }
        postToMain([count]() {
            if (g_app->emptyTrashButton)
                gtk_widget_set_sensitive(g_app->emptyTrashButton, count > 0);
        });
    }).detach();
}
...
void onEmptyTrashClicked(GtkButton*, gpointer) {
    ...
    gtk_alert_dialog_set_detail(dlg,
        "This empties the Filen trash. The items cannot be recovered.");
...
void doLogin(GtkWidget*, gpointer) {
    ...
    std::thread([e, p, t]() {
        ...
        postToMain([err]() {
            ...
            g_app->path.clear();
            g_app->path.push_back({g_app->client.baseFolderUuid(), "Home"});
            loadFolder(g_app->client.baseFolderUuid());
        });
    }).detach();
}
...
GtkWidget* buildLoginView() {
    ...
    GtkWidget* title = gtk_label_new("Sign in to Filen");
...
int main(int argc, char** argv) {
    App app;
    g_app = &app;
    g_set_application_name("Filen GUI");
